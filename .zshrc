...
export FLUTTER_HOME=$HOME/Documents/dev/flutter
export PATH=/usr/lib/dart/bin:$FLUTTER_HOME/.pub-cache/bin:$FLUTTER_HOME/bin:$PATH
...
cd-api-shaadi() { 
        cd /mnt/dev/shaadiapi && gnome-terminal --tab
}

cd-react-shaadi() {
	cd /mnt/dev/shaadi-react && gnome-terminal --tab
}
...
alias monitor-off="xrandr --output HDMI1 --off";
monitor-on() {
        xrandr --output HDMI1 --auto --${1:-"right-of"} eDP1;
};
